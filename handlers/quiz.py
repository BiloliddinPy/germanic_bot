...
async def quiz_level_handler(call: CallbackQuery, state: FSMContext):
    ...
    parts = data.split("_")
    if len(parts) < 2:
        await call.answer("Noto'g'ri daraja formati.", show_alert=True)
        return
    level = parts[1]
...
async def quiz_start_questions(call: CallbackQuery, state: FSMContext):
    ...
    parts = data.split("_")
    if len(parts) < 4:
        await call.answer("Noto'g'ri quiz parametri.", show_alert=True)
        return
    level = parts[2]
    length = int(parts[3])
    ...
    await state.update_data(questions=questions, current_idx=0, score=0, level=level)
...
async def quiz_answer_handler(call: CallbackQuery, state: FSMContext):
    ...
    if idx is None or not questions:
        await call.answer(
            "Sessiya muddati tugagan. Iltimos, qaytadan boshlang.", show_alert=True
        )
        message = call.message if isinstance(call.message, Message) else None
        if message:
            await quiz_start_handler(message, state)
        return

    callback_data = call.data or ""
    selected_answer = callback_data.replace("quiz_answer_", "")
    ...
    if not call.from_user:
        return
    LearningService.process_review_result(
        call.from_user.id, questions[idx]["word_id"], is_correct
    )
...
async def _send_next_question(call, question, idx, total):
    ...
    builder = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=opt, callback_data=f"quiz_answer_{opt}")]
...
async def _show_quiz_results(call, score, total, level):
    ...
    builder = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🔄 Qayta urinish", callback_data=f"quiz_{level}"
                )
            ],
    ...
    if not call.from_user:
        return
    StatsService.mark_progress(
        call.from_user.id, "quiz", level, completed=(percentage >= 80)
    )
    message = call.message if isinstance(call.message, Message) else None
    if not message:
        await call.answer("Xabar topilmadi.", show_alert=True)
